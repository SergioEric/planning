...
const Page: NextPage = () => {
  ...
  const { id: teamId } = router.query as { id: string };
  ...
  const { data, error } = useSWR<Daily[]>(
    `/api/getTeamDailys?startDate=${WEEKS[currentWeek].start}&endDate=${WEEKS[currentWeek].end}&teamId=${teamId}`,
    fetcher,
  ...
  return (
    <>
      ...
      <Modal open={modalIsOpen} onClose={reset} maxWidth={420} maxHeight={300}>
        {/* unmount AddNewDaily */}
        {modalIsOpen && <AddNewDaily teamId={teamId} />}
      </Modal>
                    ...
                    <button
                      ...
                      onClick={() =>
                        changeModalState({
                          ...
                          url: `/api/getTeamDailys?startDate=${WEEKS[currentWeek].start}&endDate=${WEEKS[currentWeek].end}&teamId=${teamId}`,
                        })
                      }
                    >
                      <AddIcon />
...
const ContainerPage = () => {
  ...
  if (!id || isNaN(+id)) {
    return <strong>invalid url</strong>;
  }
